...
export function ATSCandidatesTable({
  candidates,
  onCandidateUpdate,
}: ATSCandidatesTableProps) {
  ...
  const [viewingCandidates, setViewingCandidates] = useState<Set<string>>(
    new Set()
  );
  ...
  const [processingPhase, setProcessingPhase] = useState<
    "idle" | "collecting-data" | "running-analysis"
  >("idle");
  ...
  const handleManualProcessing = async () => {
    ...
    try {
      ...
      for (const candidateId of selectedCandidates) {
        ...
        if (!candidate?.HireSense_applicant_id) continue;

        // Track candidates that need AI analysis
        if (candidate.ai_status === "pending") {
          candidatesNeedingAI.push(candidate.HireSense_applicant_id);
        }
        ...
        if (candidate.cv_file_id && candidate.cv_status === "pending") {
          dataProcessingPromises.push(
            fetch("/api/cv-process", {
              ...
              body: JSON.stringify({
                applicant_id: candidate.HireSense_applicant_id,
      ...
      if (
        dataProcessingPromises.length === 0 &&
        candidatesNeedingAI.length === 0
      ) {
        return;
      }
      ...
      if (candidatesNeedingAI.length > 0) {
        ...
        const getCurrentCandidate = (
          ...
          const original = candidates.find(
            (c) => c.HireSense_applicant_id === applicantId
          );
            ...
            if (response.ok) {
              ...
              if (candidateToUpdate && onCandidateUpdate) {
                ...
                const updatedCandidate = {
                  ...
                  score:
                    analysisResult.ai_data?.score || candidateToUpdate.score,
                  analysis:
                    analysisResult.ai_data || candidateToUpdate.analysis,
                };
              ...
              console.error(
                `❌ AI analysis failed for candidate ${applicantId}:`,
                response.status
              );
            }
          } catch (error) {
            console.error(
              `❌ AI analysis error for candidate ${applicantId}:`,
              error
            );
          }
        }
  ...
  const handleBatchPushScores = async () => {
    ...
    try {
      const applicantIds = selectedCandidatesForScoring.map(
        (c) => c.HireSense_applicant_id
      );
      ...
      if (result.success) {
        ...
        alert(
          `Batch push completed: ${result.summary.successful}/${result.summary.total} successful`
        );
      } else {
  ...
  const selectedCandidatesForScoring = filteredCandidates.filter(
    (c) =>
      selectedCandidates.includes(c.id) &&
      c.HireSense_applicant_id &&
      (c.score !== undefined || c.analysis?.score !== undefined) &&
      c.ai_status === "ready" && // AI analysis must be complete
      (c.cv_status === "ready" ||
        c.li_status === "ready" ||
        c.gh_status === "ready") // At least one source processed
  );
  ...
  const handleIndividualAnalysis = async (candidate: ATSCandidate) => {
    if (!candidate.HireSense_applicant_id) {
      showNotification(
        "error",
        "This candidate is not yet linked to the analysis system"
      );
      return;
    }
    ...
    if (!isEligibleForAIAnalysis(candidate.score || 10)) {
      showNotification(
        "error",
        "AI analysis is only available for candidates with complete data (both LinkedIn and CV)"
      );
      return;
    }
                    ...
                    <div className="flex items-center gap-2">
                      {candidate.HireSense_applicant_id ? (
                        <Button
                          size="sm"
                          variant="ghost"
                          disabled={candidate.HireSense_status !== "completed"}
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(
                              `/board?id=${candidate.HireSense_applicant_id}`
                            );
                          }}
