...
export type ProcessingStatusField = 'cv_status' | 'li_status' | 'gh_status' | 'lc_status' | 'ai_status';

// Type for data fields corresponding to status fields
export type ProcessingDataField = 'cv_data' | 'li_data' | 'gh_data' | 'lc_data' | 'ai_data';
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({
        ...
        error: processingError instanceof Error ? processingError.message : `${processingName} processing failed`,
        applicant_id: applicantId
      }, { status: (processingError as any).status || 500 });
    }
