...
export async function processLinkedInUrl(
  ...
  let apiKey = process.env.BRIGHTDATA_API_KEY;
  ...
  try {
    ...
    console.log(`🚀 Sending LinkedIn request to BrightData for URL: ${targetUrl}`);
...
export async function startLinkedInJob(linkedinUrl: string): Promise<{ jobId: string; data?: unknown; isExisting?: boolean }> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Create a custom error object that mimics a response-like structure or just includes the status
      const error = new Error(`LinkedIn API request failed: ${response.status} ${response.statusText}`);
